...
class LayoutCacheService {
  ...
  /// Gets the latest layout key for a book
  Future<String?> getLatestLayoutKey(int bookId) async {
    // Query for the most recent cache entry for this book
    final cacheEntry = await _isar.pageCaches
        .filter()
        .bookIdEqualTo(bookId)
        .sortByIdDesc()
    ...
    return cacheEntry.fontSizeKey; // Using fontSizeKey as the layout key
  }

  /// Gets the layout for a specific layout key
  Future<Map<int, int>?> getLayout(String layoutKey) async {
    // Query for the cache entry with this layout key
    final cacheEntry = await _isar.pageCaches
        .filter()
        .fontSizeKeyEqualTo(layoutKey)
        .findFirst();
